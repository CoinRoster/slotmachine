...
var remainingTime=0;
var costPerSpin=2500; //gwei
var units=" gwei";

console.log(color().cursorreset);
console.log(color().clearscreen);
for (var counter=0; counter<numLoops; counter++) {
	...
	if (deltaSampleCount<=0) {
		remainingTime=deltaTime*(numLoops-counter);
		deltaSampleCount=20;
	}
	console.log("                   Spin "+color().yellow+counter+color().white+" of "+color().yellow+numLoops+color().white+clearLineEnd);	
	...
	console.log("Total wallet conributions: "+walletValue+units+" (@"+costPerSpin+"/spin)"+clearLineEnd);	
